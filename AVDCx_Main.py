...
class MyMAinWindow(QMainWindow, Ui_AVDV):
    ...
    def __init__(self, parent=None):
        ...
        # 初始化需要的变量
        self.localversion = '20211002'                                         # 当前版本号
        self.github_project_url = 'https://github.com/Hermit10/AVDCx/'         # 项目主页
        self.main_path = self.mainPath()                                       # 获取主程序路径
        self.json_data = {}
        self.img_path = ''
        self.actor_xml = ''
        self.m_drag = False
        ...
        # 启动后一些文案提示和后台检查更新
        self.Ui.label_show_version.setText('version ' + self.localversion)     # 主界面左下角显示版本信息
    ...
    def load_config(self):
        ...
        if os.path.exists(config_file):
            ...
            try:                                                               # javbus网址
                javbus_website = str(config['proxy']['javbus_website']).replace('https://', '').replace('http://', '').strip('/')
                self.Ui.lineEdit_javbus_website.setText(javbus_website)
            except:
                self.Ui.lineEdit_javbus_website.setText('')

            try:                                                               # javdb网址
                javdb_website = str(config['proxy']['javdb_website']).replace('https://', '').replace('http://', '').strip('/')
    ...
    def save_config_clicked(self):
        ...
        # ======================================================================================下载文件，剧照
        if self.Ui.checkBox_download_nfo.isChecked():                          # 下载nfo
            download_nfo = 'on'
        else:
            download_nfo = 'off'
        if self.Ui.checkBox_download_poster.isChecked():                       # 下载poster
            download_poster = 'on'
        else:
            download_poster = 'off'
        if self.Ui.checkBox_download_thumb.isChecked():                        # 下载thumb
            download_thumb = 'on'
        else:
            download_thumb = 'off'
        if self.Ui.checkBox_download_fanart.isChecked():                       # 下载fanart
            download_fanart = 'on'
        else:
            download_fanart = 'off'
        if self.Ui.checkBox_download_extrafanart.isChecked():                  # 下载extrafanart
            download_extrafanart = 'on'
        else:
            download_extrafanart = 'off'
        if self.Ui.checkBox_download_extrafanart_copy.isChecked():             # 下载剧照副本
            download_extrafanart_copy = 'on'
        else:
            download_extrafanart_copy = 'off'
        if self.Ui.checkBox_old_nfo.isChecked():                               # 保留nfo
            keep_local_nfo = 'on'
        else:
            keep_local_nfo = 'off'
        if self.Ui.checkBox_old_poster.isChecked():                            # 保留poster
            keep_local_poster = 'on'
        else:
            keep_local_poster = 'off'
        if self.Ui.checkBox_old_thumb.isChecked():                             # 保留thumb
            keep_local_thumb = 'on'
        else:
            keep_local_thumb = 'off'
        if self.Ui.checkBox_old_fanart.isChecked():                            # 保留fanart
            keep_local_fanart = 'on'
        else:
            keep_local_fanart = 'off'
        if self.Ui.checkBox_old_extrafanart.isChecked():                       # 保留extrafanart
            keep_local_extrafanart = 'on'
        else:
            keep_local_extrafanart = 'off'
        if self.Ui.checkBox_old_extrafanart_copy.isChecked():                  # 保留剧照副本
            keep_local_extrafanart_copy = 'on'
        else:
            keep_local_extrafanart_copy = 'off'
        if self.Ui.radioButton_actor_photo_upload_on.isChecked():              # 自动上传头像
            actor_photo_upload = 'on'
        elif self.Ui.radioButton_actor_photo_upload_off.isChecked():
    ...
    def pushButton_select_file_clicked(self):
        ...
        file_path, filetype = QtWidgets.QFileDialog.getOpenFileName(self, "选取视频文件", media_path, "Movie Files(*.mp4 " "*.avi *.rmvb *.wmv " "*.mov *.mkv *.flv *.ts " "*.webm *.MP4 *.AVI " "*.RMVB *.WMV *.MOV " "*.MKV *.FLV *.TS " "*.WEBM);;All Files(*)")
    ...
    def pushButton_select_thumb_clicked(self):
        ...
        if not path:
            path = self.main_path
        file_path, fileType = QtWidgets.QFileDialog.getOpenFileName(self, "选取缩略图", path, "Picture Files(*.jpg *.png);;All Files(*)")
        if file_path != '':
    ...
    # 自动上传头像
    def uploadActorPhoto_main(self):
    ...
    # 找到可以上传的演员，然后调用上传函数
    def found_actor_photo(self, json_data, local_actor_photo_list):
    ...
    # 上传头像
    def upload_actor_photo(self, count, actor, pic_path):
        emby_url = self.Ui.lineEdit_emby_url.text()
        api_key = self.Ui.lineEdit_api_key.text()
        emby_url = emby_url.replace('：', ':').strip('/')
        if 'http://' not in emby_url and 'https://' not in emby_url:
    ...
    def extrafanartDownload(self, json_data, folder_new_path):
        ...
        for extrafanart_url in extrafanart_list:
            ...
            i = 1
            while i <= self.config.get('retry'):
                self.downloadFileWithFilename(extrafanart_url, 'fanart' + str(extrafanart_count) + '.jpg', extrafanart_path)
    ...
    def PrintFiles(self, nfo_new_path, folder_new_path, thumb_new_name, poster_new_name, fanart_new_name, json_data):
        ...
        name_media = json_data['naming_media'].replace('title', title).replace('studio', studio).replace('year', year).replace('runtime', runtime).replace('director', director).replace('actor', actor).replace('release', release).replace('number', number).replace('series', series).replace('publisher', publisher).replace('mosaic', mosaic).replace('definition', definition).replace('cnword', c_word)
        # 字符转义，emby避免无法解析
        ...
        try:
            ...
            with open(nfo_new_path, "wt", encoding='UTF-8') as code:
                ...
                # 输出评分
                try:
                    if str(json_data['score']) and str(json_data['score']) != '' and float(json_data['score']) != 0.0:
    ...
    def showMovieInfo(self, json_data):
        if self.config.get('switch') == 0:                                     # 调试模式打开时显示详细日志
            return
        escape_key = ['image_download', 'image_cut', 'search_url', 'log_info', 'error_type', 'error_info', 'naming_media', 'naming_file', 'folder_name', 'extrafanart', 'actor_photo', 'source', 'cover', 'number', 'cover_small', 'req_web']
        try:
            for key, value in json_data.items():
                if value == '' or key in escape_key:
                    continue
                if len(str(value)) == 0:
                    continue
    ...
    def getFolderPath(self, file_path, success_folder, json_data):
        ...
        folder_new_name = folder_name.replace('title', title).replace('studio', studio).replace('year', year).replace('runtime', runtime).replace('director', director).replace('actor', actor).replace('release', release).replace('number', number + c_word).replace('series', series).replace('publisher', publisher).replace('mosaic', mosaic).replace('definition', definition).replace('cnword', cnword)
    ...
    def getNamingRule(self, file_path, json_data):
        ...
        # 获取文件名
        file_name = json_data['naming_file'].replace('title', title).replace('studio', studio).replace('year', year).replace('runtime', runtime).replace('director', director).replace('actor', actor).replace('release', release).replace('number', number).replace('series', series).replace('publisher', publisher).replace('mosaic', mosaic).replace('definition', definition).replace('cnword', cnword)
    ...
    def netResult(self):
        ...
        net_info = [
            ['github', 'https://github.com', ''], ['iqqtv', 'https://iqqtv.cloud', ''], ['javbus', javbus_url, ''], ['javdb', javdb_url, ''], ['jav321', 'https://www.jav321.com', ''], ['dmm', 'https://www.dmm.co.jp', ''], ['avsox', 'https://avsox.website', ''], ['xcity', 'https://xcity.jp', ''], ['mgstage', 'https://www.mgstage.com', ''], ['fc2', 'https://adult.contents.fc2.com', ''],
            ['fc2hub', 'https://fc2hub.com', ''], ['airav', 'https://www.airav.wiki', ''], ['javlibrary', 'http://www.javlibrary.com', '']
        ]
    ...
    def youdao_translate(self, title, outline):
        ...
        Form_Data = {'i': msg, 'from': 'AUTO', 'to': 'zh-CHS', 'smartresult': 'dict', 'client': 'fanyideskweb', 'salt': salt, 'sign': sign, 'ts': ts, 'bv': 'c6b8c998b2cbaa29bd94afc223bc106c', 'doctype': 'json', 'version': '2.1', 'keyfrom': 'fanyi.web', 'ue': 'UTF-8', 'typoResult': 'true', 'action': 'FY_BY_CLICKBUTTION'}
    ...
    def transLanguage(self, movie_number, jsonfile_data, json_data, translate_language, translate_content, translate_by):
        ...
        if trans_title or trans_outline:
            if translate_by == 'youdao':                                       # 使用有道翻译
                ...
                trans_title, trans_outline = self.youdao_translate(trans_title, trans_outline)
                if trans_title:
    ...
    def getInfoData(self, json_data):
        ...
        if info_output == 'zh_cn':
            if series:                                                         # 为空时会匹配所有
                series_output = info_xml.xpath('a[contains(@keyword, $name)]/@zh_cn', name=series_name)
                if series_output and series_output[0] != '删除':
                    series = series_output[0]
                tag += ',系列：' + series
            if studio:
                studio_output = info_xml.xpath('a[contains(@keyword, $name)]/@zh_cn', name=studio_name)
            ...
            if not publisher:
                publisher = studio
            if publisher:
                publisher_output = info_xml.xpath('a[contains(@keyword, $name)]/@zh_cn', name=publisher_name)
                if publisher_output and publisher_output[0] != '删除':
                    publisher = publisher_output[0]
                tag += ',发行：' + publisher
            if director:
                director_output = info_xml.xpath('a[contains(@keyword, $name)]/@zh_cn', name=director_name)
        ...
        # 中文繁体
            ...
            if series:                                                         # 为空时会匹配所有
                series_output = info_xml.xpath('a[contains(@keyword, $name)]/@zh_tw', name=series_name)
                if series_output and series_output[0] != '删除':
                    series = series_output[0]
                tag += ',系列：' + series
            if studio:
                studio_output = info_xml.xpath('a[contains(@keyword, $name)]/@zh_tw', name=studio_name)
                if studio_output and studio_output[0] != '删除':
                    studio = studio_output[0]
                tag += ',製作：' + studio
            if not publisher:
                publisher = studio
            if publisher:
                publisher_output = info_xml.xpath('a[contains(@keyword, $name)]/@zh_tw', name=publisher_name)
                if publisher_output and publisher_output[0] != '删除':
                    publisher = publisher_output[0]
                tag += ',發行：' + publisher
            if director:
                director_output = info_xml.xpath('a[contains(@keyword, $name)]/@zh_tw', name=director_name)
                if director_output and director_output[0] != '删除':
                    director = director_output[0]
        ...
        # 日语
            ...
            if series:                                                         # 为空时会匹配所有
                series_output = info_xml.xpath('a[contains(@keyword, $name)]/@jp', name=series_name)
                if series_output and series_output[0] != '删除':
                    series = series_output[0]
                tag += ',系列：' + series
            if studio:
                studio_output = info_xml.xpath('a[contains(@keyword, $name)]/@jp', name=studio_name)
                if studio_output and studio_output[0] != '删除':
                    studio = studio_output[0]
                tag += ',製作：' + studio
            if not publisher:
                publisher = studio
            if publisher:
                publisher_output = info_xml.xpath('a[contains(@keyword, $name)]/@jp', name=publisher_name)
                if publisher_output and publisher_output[0] != '删除':
                    publisher = publisher_output[0]
                tag += ',發行：' + publisher
            if director:
                director_output = info_xml.xpath('a[contains(@keyword, $name)]/@jp', name=director_name)
                if director_output and director_output[0] != '删除':
                    director = director_output[0]
    ...
    def move_torrent(self, folder_old_path, folder_new_path, file_name, movie_number, naming_rule):
        ...
        if os.path.exists(torrent_file1):
            shutil.move(torrent_file1, os.path.join(folder_new_path, (naming_rule + '.torrent')))
            self.addTextMain(" 🟢 Torrent file '%s' moved successfully! " % (naming_rule + '.torrent'))

        elif os.path.exists(torrent_file2):
            shutil.move(torrent_file2, os.path.join(folder_new_path, (movie_number + '.torrent')))
            self.addTextMain(" 🟢 Torrent file '%s' moved successfully! " % (movie_number + '.torrent'))
    ...
    def coreMain(self, file_path, file_info, file_mode, appoint_number='', appoint_url='', jsonfile_data={}):
        ...
        if self.config.get('main_mode') == 2:
            # 整理模式（仅根据刮削数据把电影命名为番号并分类到对应目录名称的文件夹下）
            if not self.pasteFileToFolder(file_path, file_new_path):
                # 移动文件
                return False, json_data
                # 返回AVDC_main, 继续处理下一个文件
            else:
    ...
    def AVDC_Main(self, file_mode):
        ...
        for file_path in movie_list:
            try:
                ...
                self.addTextMain('movie_list: %s ' % e)
            count += 1
            # 获取进度
            ...
            # 显示刮削信息
            self.set_label_file_path.emit('正在刮削： %s/%s %s \n %s' % (str(count), str(count_all), progress_percentage, self.convert_path(file_show_path)))
